...
def _summarize_and_finish(job_id: str, transcript: str) -> None:
...
def run_summarize_job(job_id: str, url: str) -> None:
    ...
    _summarize_and_finish(job_id, transcript)


def run_summarize_text_job(job_id: str, transcript: str) -> None:
    _summarize_and_finish(job_id, transcript)
...
@router.post("/summarize/text", response_model=JobCreatedResponse, status_code=202)
def summarize_from_text(request: SummarizeTextRequest, background_tasks: BackgroundTasks):
    job = create_job()
    background_tasks.add_task(run_summarize_text_job, job.id, request.transcript)
    return JobCreatedResponse(job_id=job.id)
